...
export const AGENT_A_GENE_TEMPLATE = {
  ...
  summary: '修复 React useEffect 依赖缺失的策略',
  strategy: [
    '识别所有 state 变量',
    '识别所有 useCallback 回调',
    '将回调加入依赖数组',
  ...
  validation: ['npx vitest run tests/useEffect.test.js'],
};
...
export function makeAgentBCapsuleTemplate(reusedAssetId, parentAssetId) {
  return {
    ...
    content:
      '策略：使用 useCallback 包装所有回调，将所有回调加入依赖数组。结果：测试通过。lesson：useCallback 回调必须加入 useEffect 依赖数组。',
...
export function makeAgentCCapsuleTemplate(reusedAssetId, parentAssetId) {
  return {
    ...
    content:
      '策略：直接复用 capsule_success_001 的方案——使用 useCallback 包装所有回调，将所有回调加入依赖数组。结果：测试通过。复用让修复从分钟级降到秒级。',
    diff: '--- a/Component.jsx\n+++ b/Component.jsx\n@@ -10,6 +10,15 @@\n+const handleClick = useCallback(() => { ... }, [dep]);\n useEffect(() => { handleClick(); }, [handleClick]);',
    confidence: 0.95,
    blast_radius: { files: 2, lines: 15 },
